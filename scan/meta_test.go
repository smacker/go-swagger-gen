...
func verifyMeta(t testing.TB, doc *spec.Swagger) {
	...
	assert.EqualValues(t, []map[string][]string{{"api_key": {}, "api_key_2": {}}}, doc.Security)
	...
	expectedSecuritySchema2 := spec.SecurityScheme{
		SecuritySchemeProps: spec.SecuritySchemeProps{
			Type: "apiKey",
			In:   "header",
			Name: "KEY2",
	...
	assert.EqualValues(t, map[string]*spec.SecurityScheme{"api_key": &expectedSecuritySchema, "api_key_2": &expectedSecuritySchema2}, doc.SecurityDefinitions)
